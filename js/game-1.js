...
const gameOneElement = getElementFromHtmlString(`<div>
  <header class="header">
...
export default () => {
  const renderedElement = renderElement(gameOneElement);
  const backButton = renderedElement.querySelector(`.back`);
  const answers1 = renderedElement.querySelectorAll(`input[name="question1"]`);
  const answers2 = renderedElement.querySelectorAll(`input[name="question2"]`);
  let isQuestion1Answered = false;
  let isQuestion2Answered = false;
  backButton.addEventListener(`click`, () => renderRulesElement());
  const answers = renderedElement.querySelectorAll(`.game__answer`);
